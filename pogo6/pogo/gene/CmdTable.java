...
//
// $Log$
// Revision 3.14  2007/11/19 10:25:16  pascal_verdier
// Some spelling mistakes fixed.
//
// Revision 3.13  2006/12/05 09:33:52  pascal_verdier
...
//===============================================================
    private void parsePyCommands(PogoString code) throws PogoException
    {
		...
		//	It changes again to be same as cpp (IDL4) !!!
		Cmd	cmd;
		cmd = cmdAt(0);
		if (code.indexOf("def dev_state(self):")>0)
		...
		cmd = cmdAt(1);
		if (code.indexOf("def dev_status(self):")>0)
			cmd.override_method = ALREADY_OVERRIDED;
		else
        ...
        //  Search command list
        int start = code.str.indexOf("Class(PyTango.DeviceClass):");
        if (start<0)
        	start = code.str.indexOf("Class(PyTango.PyDeviceClass):");
        if (start<0)
        ...
        while ((start=cmd_list.indexOf(target, end))>0)
        {
            ...
            //	Create a new command object
            cmd = new Cmd(name, name, argin_type, argout_type, argin_desc, argout_desc, "");
            ...
            //  In python the convention has changed !!!! exec method i the command's name
			//	It changes again to be same as cpp (IDL4) for state and status !!!
			if (cmd.name.equals("Status")==false &&
				cmd.name.equals("State")==false )
			{
...
//=======================================================================
/**
 *	Buid description table to give correspondence between commands and method's name.
 *
 ...
 */
 //=======================================================================
	protected String addCommentsTable()
	{
        ...
        for (int j="Command name".length() ; j<max ; j++)
			str += " ";
		str += "|  Method name\n";
		str += "//	----------------------------------------\n";
		for (int i=0 ; i<size() ; i++)
