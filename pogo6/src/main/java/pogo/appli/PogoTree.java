...
//
// $Log$
// Revision 3.25  2009/02/18 15:03:48  pascal_verdier
// Tango 7 (IDL 4) compatibility implementation.
//
// Revision 3.24  2007/11/19 10:22:31  pascal_verdier
...
public class  PogoTree  extends JTree implements PogoAppliDefs, PogoDefs
{
	...
	//======================================================
	//======================================================
	void setDeviceID(DeviceID device_id)
	{
		server.device_id = device_id;
	}
	//======================================================
	//======================================================
	void editDeviceClassID()
	{
		server.device_id.classname = server.class_name;
		DeviceIdDialog	dlg =
			new DeviceIdDialog(parent, server.device_id, false);
		if (dlg.showDialog()==JOptionPane.OK_OPTION)
			server.device_id = dlg.getInputs();
	}
	//======================================================
	//======================================================
	void showClassSummary()
	{
		PogoClass	server = getPogoClass();
		String		summary = "Class:  " + server.class_name + "\n";
       	if (!server.inheritedClassName().equals(PogoDefs.tangoDeviceImpl))
			summary += "Inherited from:  " + server.inheritedClassName() + "\n";
		if (server.author!=null && server.author.length()>0)
			summary += "Author:  " + server.author + "\n";
		if (server.revision!=null && server.revision.length()>0)
			summary += "Revision:  " + server.revision + "\n";

		summary += " \n";
     	summary += "   - " + 
			server.class_properties.size() + " class properties\n";
     	summary += "   - " + 
			server.dev_properties.size() + " device properties\n";
     	summary += "   - " + 
			server.commands.size() + " commands\n";
     	summary += "   - " + 
			server.attributes.size() + " attributes\n";
     	summary += "         - " + 
			server.attributes.getScalarNumber() + " scalar(s)\n";
     	summary += "         - " + 
			server.attributes.getSpectrumNumber() + " spectrum(s)\n";
     	summary += "         - " + 
			server.attributes.getImageNumber() + " image(s)\n";
     	summary += "   - " + 
			server.states.size() + " states\n";
		app_util.PopupMessage.show(parent, summary);
	}
	//======================================================
	/**
	...
	public PogoClass getPogoClass()
	{
		...
		try
		{
            ...
            new_server.device_id        = server.device_id;
			new_server.device_id.classname = server.class_name;	//	Could have changed.
		}
