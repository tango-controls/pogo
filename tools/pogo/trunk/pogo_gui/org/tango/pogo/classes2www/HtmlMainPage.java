//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/pogo/classes2www/HtmlMainPage.java,v $
//
// Project:   Tango
//
...
public class  HtmlMainPage
{
	...
	private void generateList()
	{
		...
		try {
			...
			ParserTool.writeFile(path + "/" + IHtmlTemplates.html_list, sb.toString());
		}
		catch(DevFailed e) {
	...
	private String getBottomList()
	{
		...
		for (String s : link_to)
		{
			...
			item = Utils.strReplace(item, "$LINK", link);
			item = Utils.strReplace(item, "$TEXT", rep.name + " Classes");
	...
	private void generateIndex()
	{
		...
		str = Utils.strReplace(str, "$PATH", path );
		sb.append(str);
		try {
			ParserTool.writeFile(repository.home + "/" + IHtmlTemplates.html_index, sb.toString());
		}
		catch(DevFailed e) {
			Except.print_exception(e);
		}
	...
	private void generateAlphabetical()
	{
		...
		try {
			Repository	repository = repositories.getRepository(repos_names[0]);
			ParserTool.writeFile(repository.home + "/" + IHtmlTemplates.html_alpha, sb.toString());
		}
		catch(DevFailed e) {
			Except.print_exception(e);
		}
