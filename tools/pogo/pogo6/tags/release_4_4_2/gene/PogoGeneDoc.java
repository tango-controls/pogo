...
//=======================================================================
	public PogoGeneDoc(PogoClass server)
	{
		super(server);
		this.server = server;
		outDoc = new String(server.projectFiles.getDocHtml());
		header = new String("\"" + server.class_name +
...
//=======================================================================
	private String[] buildCmdDocFromCpp() throws 
										...
										PogoException
	{
		String	template  = new String(server.templates_dir + "/html/Doxyfile");
		String	doxyfile  = new String(server.projectFiles.getPath() + "/Doxyfile");
		...
		code.replace("POGO_TITLE",     "\"" + server.title + "\"");
		code.replace("POGO_SRC_FILES", server.projectFiles.getPath());
...
//=======================================================================
	...
	{
		String[]	cmd = new String[server.commands.size() + 9];
		int	n = 0 ;
		cmd[n++] = "javadoc";
		cmd[n++] = "-private";
		cmd[n++] = "-author";
		...
		cmd[n++] = "-d";
		cmd[n++] = outDoc + "/html";
		cmd[n++] = new String(server.projectFiles.getPath() + "/" +
							server.class_name + javaExtention);
		cmd[n++] = new String(server.projectFiles.getPath() + "/" +
							server.class_name + "Class" + javaExtention);
		for (int i=2 ; i < server.commands.size() ; i++)
		{
			Cmd		c = server.commands.cmdAt(i);
			cmd[n++] = new String(server.projectFiles.getPath() + "/" +
...
//=======================================================================
						...
						throws	IOException, PogoException
	{
		...
		if (server.inherited_from!=null &&
			server.inheritedClassName().equals(tangoDeviceImpl)==false)
		{
...
//=======================================================================
						...
						throws	IOException, PogoException
	{
		...
		FileInputStream		fidin 
			= new FileInputStream(server.templates_dir + "/html/" + filename);
		int nb = fidin.available();
		byte[]	inStr  = new byte[nb];
		fidin.read(inStr);
