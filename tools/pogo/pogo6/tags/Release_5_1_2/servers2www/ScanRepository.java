...
//
// $Log$
// Revision 1.11  2007/04/17 14:06:40  pascal_verdier
// The server family is now specified by ### in tango-ds modules file.
//
// Revision 1.10  2006/11/09 14:11:20  pascal_verdier
// Link on sourceforge added.
// Language added.
//
// Revision 1.9  2006/04/05 12:32:25  pascal_verdier
...
public class  ScanRepository implements PogoDefs
{
	...
	private void scanTangoDsModuleFile(String repository) throws Exception
	{
		...
		String	target = "###";		//	Header for family
		for (int i=0 ; i<lines.length ; i++)
		{
			if (lines[i].indexOf("#")!=0)	//	Not a comment
			{
				if (lines[i-1].indexOf(target)==0) //Take previous comment as family
				{
	...
	public int cvsCheckOutModules() throws Exception
	{
		...
		Vector	tags = PogoUtil.getLastCvsTags(cvsroot);
		//	For each project
		for (int i=0 ; i<nb_modules() ; i++)
		{
			//	Get module name
