...
//============================================================================
	public Cmd(String cmdLine) throws PogoException
	{
		...
		if (fields.length>=7)
		{
			if (fields[6].equals("Tango::EXPERT"))
...
//============================================================================
	private String buildExecCmdMethodName()
	{
		...
		{
			if (command.charAt(i) >= 'A' && command.charAt(i) <= 'Z')//	if upperr case
			{
				if (i>0)
				...
				//---------------------------
				sb.append ((char)(command.charAt(i) + ('a'-'A')));
			}
			else
...
//============================================================================
	...
	{
		String	target = new String("if (command_list[i]->get_name()==\""
									+ command + "\")\n" +
						"			command_list[i]->set_polling_period(");
		//	Search if command is polled
		int	start;
		if ((start=cmd_factory.indexOf(target))>0)
		{
...
//============================================================================
/**
 * Get the execute method's name from C++ source files if exist 
 * or does'nt change.
 *
 * @param class_name		The class name for this method.
 * @exception PogoException If synthax error is detected in input file.
 */
//============================================================================
	public void checkCppExecCmdMethodName(String filename)
								throws	PogoException
	{
		try
		{
			//	Get The Ouput file directory from the data file
			//--------------------------------------------------
			FileInputStream fid = new FileInputStream(filename);

			//	Get nb data to read to allocate byte array
			//---------------------------------------------
			...
			//	Check exec method name already written
			//	and compeare with the theoric one.
			//---------------------------------------------
			int	start, end;
			String pattern = new String("CORBA::Any *" + cmd_class + "::execute");
			if ((start=str.indexOf(pattern))<0)
			{
			...
			if (exec_method.equals(name)==false)
			{
				System.out.println(name + ": Not conventionnal name !");
				System.out.println("It should be :   "+ exec_method + "()...");
				exec_method = new String(name);
			}
		}
		catch (FileNotFoundException ex)
		{
			//	File Not Found Do not change exec method
...
//============================================================================
/**
 * Get the execute method's name from Java source files if exist 
 * or does'nt change.
 *
 * @param filename		File where check the execute method.
 * @exception PogoException If synthax error is detected in input file.
 */
//============================================================================
	public void checkJavaExecCmdMethodName(String filename)
								throws	PogoException
	{
		try
		{
			...
			for (end=0 ; (start=str.indexOf(pattern, end))>0 ; )
			{
				start += pattern.length();
				end = str.indexOf("(", start);
				String name = str.substring(start, end).trim();
				
				if (exec_method.equals(name)==false)
				{
					System.out.println("\"" + exec_method + "\"");
					exec_method = new String(name);
					System.out.println(exec_method + ": Not conventionnal name !\n");
				}
			}
		}
		catch (FileNotFoundException ex)
...
//============================================================================
	public String AddCmdPollingLine(int lang)
	{
		...
		if (polled_period>0)
		{
			sb.append("\t\tif (command_list[i]->get_name()==\"" + command + "\")\n");
			sb.append("\t\t\tcommand_list[i]->set_polling_period("+
													polled_period + ");\n");
		}
		return sb.toString();
	}
...
//============================================================================
	...
	{	
		...
		sb.append("/**\n");
		sb.append(" *	Execute command \""+command+"\" on device.\n");
	...
	public String buildJavaProxyMethod()
	{
		...
		else
		{
			...
			if (argin.code!=Tango_DEV_VOID)
			{
				...
				sb.append("		data_out = command_inout(\"" + command + "\", data_in);\n");
			}
			else
				sb.append("		data_out = command_inout(\"" + command + "\");\n");
...
//============================================================================
	public String buildCppExecCmdMethodComments(String class_name)
	{
		...
		sb.append(" *\n");
		sb.append(" *\tdescription:	method to execute \"" + command + "\"\n");
		if (description!=null && description.length()>0)
			sb.append(" *\t" + new PogoString(description).setComments()+"\n");
		sb.append(" *\n");
		if (argin.code!=Tango_DEV_VOID)
