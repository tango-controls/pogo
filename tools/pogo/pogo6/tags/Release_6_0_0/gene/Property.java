...
//
// $Log$
// Revision 3.11  2008/03/13 08:47:27  pascal_verdier
// Default property values added to html pages.
// Bug in property description (use cotes in python) fixed.
//
// Revision 3.10  2008/01/17 10:27:55  pascal_verdier
// Bug with quotes in dscription fixed.
//
// Revision 3.9  2007/09/14 14:34:04  pascal_verdier
// Manage double cotes in description.
//
...
//======================================================================
	String geneDefaultValue(int source)
	{
		...
		while (pgs.str.indexOf("\n")>0)
			pgs.replace("\n", "\\n");
        //  Add a back slash before double cote if any 
        int idx = 0;
        while ((idx=pgs.str.indexOf("\"", idx))>0)
		{
            pgs.str = pgs.str.substring(0, idx)  +  "\\" +
                      pgs.str.substring(idx);
			idx += 2;
		}
		String	code = "";
		code += "	prop_name = \"" + name + "\";\n";
...
//======================================================================
    String buildPyFactoryLine()
    {
        ...
        idx = -2;
        while ((idx=desc.indexOf("\"", idx+2))>0)
            desc = desc.substring(0, idx) + "\\" + desc.substring(idx);
...
//======================================================================
//=========================================d============================
    String defaultValueToHtmlString()
    {
        if (default_value==null || default_value.length()==0)
            return "No default value";
        else
        if (type.code==TangoConst.Tango_DEV_STRING)
            return default_value;
        else
        {
            //  split in elements if any
            StringTokenizer stk = new StringTokenizer(default_value, "\n");
            Vector  v = new Vector();
            while (stk.hasMoreTokens())
                v.add(stk.nextToken());
