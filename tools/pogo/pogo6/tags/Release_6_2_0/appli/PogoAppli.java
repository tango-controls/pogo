...
//
// $Log$
// Revision 3.49  2009/10/07 12:42:48  pascal_verdier
 ...
 */
private static String revNumber = "Release 6.2.0  -  Mon Nov 23 13:10:31 CET 2009";
/**
...
//===========================================================
  void createNewServer()
  {
	...
	//	Before everything, check if device id already filled.
	DeviceIdDialog	id_dlg = new DeviceIdDialog(this, "", false);
	if (id_dlg.showDialog()==JOptionPane.CANCEL_OPTION)
		return;
	...
	if (classDialog.showDialog()== PogoAppliDefs.RET_OK)
	{
		...
		try
		{
			...
			pogo.device_id = id_dlg.getInputs();
				...
				if (!file.isDirectory())
      			{
					...
					String	id_filename = DeviceID.getFilename(file.getAbsolutePath());
					if (new File(id_filename).exists()==false)
					{
						String	classname = file.getName();
						int		pos = classname.lastIndexOf('.');
						if (pos>0)	classname = classname.substring(0, pos);
						DeviceIdDialog	id_dlg = new DeviceIdDialog(this, classname, false);
						if (id_dlg.showDialog()==JOptionPane.OK_OPTION)
						{
							//	Write file display message to do not forget commit
							id_dlg.writeFile(id_filename);
							device_id = id_dlg.getInputs();
							//	display message to do not forget commit
							if (DeviceID.useRepository(id_filename))
								app_util.PopupMessage.show(this,
