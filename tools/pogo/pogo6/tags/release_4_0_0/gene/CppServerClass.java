...
public class CppServerClass extends PogoGene implements PogoDefs, TangoConst
{
	...
	public CppServerClass(PogoClass pogo)
	{
		...
		this.is_superclass = pogo.is_superclass;
	}
...
//=========================================================================
	...
	//================================================================
	/**
	 *	Convert the C++ Server.h file, to a SuperClass.h file
	 */
	//================================================================
	private PogoString toSuperClassHeader(PogoString code)
	{
										...
										PogoException
	{
		...
		if (already_exists(serverclass_h))
		{
			...
			//--------------------------
			int	start, end;
			String	namespace = "namespace " + pogo.class_name;
			if ((end=pgs.str.indexOf(namespace))<0)
		...
		if (is_superclass)
		{
			new_code = toSuperClassHeader(new_code);
		}
...
//=========================================================================
										...
										PogoException
	{
		...
		if (is_superclass)
			factories = toSuperClass(factories);
	...
	//================================================================
	/**
	 *	Convert the C++ ServerClas.cpp file, to a SuperClass.cpp file
	 */
	//================================================================
	private String toSuperClass(String str) throws PogoException
	{
...
//=======================================================================
						...
						throws	PogoException
	{
		...
		//-----------------------------
		int	idx = 0 ;
		if ((idx=pgs.str.indexOf(new String ("namespace " + pogo.class_name)))<0)
		...
		//new_cmd.append(cmd.setCmdClass(construct_templ));		
			
		String	s = cmd.setCmdClass(exec_templ);
		new_cmd.append(cmd.buildCppCmdClassBlock(s, pogo.class_name));
		
		StringBuffer	sb = new StringBuffer(pgs.str);
		sb.insert(idx, new_cmd);
		return sb.toString();
	}
//=======================================================================
/**
...
//===============================================================
										...
										PogoException
	{
		...
		//	as a string array.
		int	start = 0;
		String	desc = new String(pogo.class_desc);
		String	newcode = "vector<string>	str_desc;\n";
		start = 0;
		int	end;
		while ((end=desc.indexOf('\n', start))>=0)
		{
			newcode += "\tstr_desc.push_back(\"";
			newcode	+= desc.substring(start, end) + "\");\n";
			start = end + 1;
		}
		newcode += "\tstr_desc.push_back(\"";
