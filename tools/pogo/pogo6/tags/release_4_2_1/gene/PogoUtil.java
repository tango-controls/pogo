...
public class PogoUtil implements PogoDefs {
					...
					IOException
	{
		...
		String superclass = readcode.substring(start, end).trim();
		start = superclass.indexOf("::");
		if (start<0)
			throw new PogoException("Inheritance syntax error");
		superclass = superclass.substring(start+2);

		//	Search the Super class files
		String[]	cl_list = superClassList();
		boolean		found = false;
		...
		for (int i=0 ; !found && i<cl_list.length ; i++)
		{
			...
			if (start>0)
			{
				...
				found = _class.equals(superclass);
				//	Ok keep the full path but not the extention
				superclass = cl_list[i].substring(0, cl_list[i].lastIndexOf("."));
			}
		}
		if (!found)
		{
...
"<P><!-------TITLE------></P>\n\n";
	...
	//===============================================================
	//===============================================================
	static private Vector searchSuperClassList(String dirname)
	{
		String[]	ext = { ".java", ".h" };
		Vector 		v = new Vector();
		if (dirname==null)
		...
		for (int i=0 ; i<filenames.length ; i++)
		{
			...
			if (f.isDirectory())
			{
				Vector	vd = searchSuperClassList(f.toString());
