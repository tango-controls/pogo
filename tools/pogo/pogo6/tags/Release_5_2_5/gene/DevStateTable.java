...
//
// $Log$
// Revision 3.8  2006/11/24 14:28:18  pascal_verdier
// Implements first Python gurus remarks.
//
// Revision 3.7  2006/11/02 11:41:00  pascal_verdier
...
//============================================================================
    void setPyNotAllowedFor(String code, String name)
    {
        ...
        target = "self.get_state() in [";
        start  = method.indexOf(target);
        if (start <0)
            return;
        start += target.length();
        end    = method.indexOf(']', start);
        ...
        while ((start=str.indexOf(target, end))>=0 && end>=0)
        {
            ...
            if ((end=str.indexOf(",", start))<0)
                statename = str.substring(start).trim();
            else
                statename = str.substring(start, end).trim();
            add(new DevState(statename, ""));
        }
    }
...
//============================================================================
    String pyUpdateAllowedStateMethod(String code, String signature)
    {
        ...
        if (start<0)
            return code;
        //  Search is_allowed test part
        start  += signature.length() + 1;
        String  target = "#\tEnd of Generated Code";
        int     end = code.indexOf(target, start);
        end = code.lastIndexOf('\n', end)+1;
        //  Then remove
        if (end>start)
            code = code.substring(0, start) +
                   code.substring(end);
        //  If no forbiden states -> nothing to be added
        if (size()==0)
        {
            //  Remove the return false if exists
            start = code.indexOf("#\tRe-Start of Generated Code");
            start = code.indexOf("return False", start);
            if (start>0)
            {
                end = code.indexOf("\n", start);
        ...
        //  Add return False if does not exist
        target = "#\tRe-Start of Generated Code";
        start = code.indexOf(target);
        start += target.length();
        if (code.indexOf("return False", start)<0)
        {
