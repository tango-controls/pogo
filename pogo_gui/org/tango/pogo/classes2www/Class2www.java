//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/pogo/classes2www/Class2www.java,v $
//
// Project:   Tango
//
...
/** 
 *	This class is able to cegt classes from 3 repositories,
 *	load them with pogo, generate documentation and put them on WEB site.
 *
 * @author  verdier
 */
...
public class  Class2www
{
	...
	public void logAuthors()
	{
		...
		try {
			ParserTool.writeFile(backup_home + "/log/authors.txt", authors.toString());
		}
		catch(Exception e) {
			System.err.println(e);
		}
	...
	{
		Authors	authors = new Authors();
		for (Family family : esrf.families) {
			for (Module module : family) {
				if (module.server!=null) {
					if (module.server.author!=null &&
						module.server.author.length()>0) {
		...
		try {
			ParserTool.writeFile(backup_home + "/log/esrf_authors.txt", authors.toString());
		}
		catch(Exception e) {
			System.err.println(e);
		}
